    ...
    <div className="py-8 bg-[#000000] text-slate-100 min-h-[85vh]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div className="bg-[#09090b] border border-[#1f1f23] rounded-3xl p-6 shadow-2xl flex flex-col md:flex-row md:items-center justify-between gap-6 backdrop-blur-md">
          ...
          <div className="flex items-center gap-4 bg-[#000000] p-4 rounded-2xl border border-emerald-500/30 shrink-0 shadow-lg">
            <div className="text-right">
              <p className="text-[10px] text-yellow-400 font-extrabold uppercase tracking-wider">Auto Progress</p>
              <p className="text-2xl font-black text-emerald-400">{calculatedProgress}% Done</p>
              <p className="text-[11px] text-zinc-400">{completedCount} of {totalPortions} Portions Completed</p>
            </div>

            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-emerald-400 via-green-500 to-yellow-400 text-slate-950 flex items-center justify-center font-black text-lg shadow-md">
              {calculatedProgress}%
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#09090b] border border-[#1f1f23] rounded-2xl p-5 shadow-xl grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-[10px] font-bold text-yellow-400 uppercase mb-1 flex items-center gap-1">
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-[#000000] border border-[#27272a] rounded-xl px-3.5 py-2.5 text-sm font-bold text-white focus:outline-none focus:border-yellow-400 cursor-pointer shadow-inner"
            >
              {BOARDS_LIST.map(b => (
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-400 via-green-500 to-yellow-400 hover:from-emerald-300 hover:to-yellow-300 text-slate-950 font-black text-xs shadow-lg shadow-emerald-500/20 transition-all shrink-0"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {currentFilteredPortions.length > 0 ? (
            currentFilteredPortions.map((portion) => (
              <div
                key={portion.id}
                className="bg-[#09090b] border border-[#1f1f23] rounded-2xl p-5 hover:border-emerald-500/40 transition-all flex flex-col justify-between shadow-xl"
              >
                <div>
                  <div className="flex items-center justify-between gap-2 mb-2">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      <span className="text-[10px] font-extrabold uppercase px-2 py-0.5 rounded bg-yellow-500/15 text-yellow-400 border border-yellow-500/30">
                        {portion.board}
                      </span>
                    </div>
                ...
                <div className="mt-4 pt-3 border-t border-[#1f1f23] flex items-center justify-between">
                  <label className="text-[10px] font-bold text-yellow-400 uppercase">Status:</label>
                  ...
                  <select
                    ...
                    className={`px-3 py-1.5 rounded-xl text-xs font-extrabold focus:outline-none transition-colors cursor-pointer border ${
                      portion.status === 'Completed' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/40' :
                      portion.status === 'In Progress' ? 'bg-yellow-500/15 text-yellow-400 border-yellow-500/40' :
                      'bg-[#000000] text-zinc-400 border-[#27272a]'
                    }`}
                  >
                    <option value="Completed" className="bg-[#09090b] text-emerald-400 font-bold">Completed</option>
                    <option value="In Progress" className="bg-[#09090b] text-yellow-400 font-bold">In Progress</option>
          ...
          <div className="bg-[#09090b] border border-[#1f1f23] rounded-3xl p-6 max-w-md w-full shadow-2xl">
            ...
            <form onSubmit={handleAddSubmit} className="space-y-4">
              <div className="bg-[#000000] p-3 rounded-xl border border-[#27272a] text-xs space-y-1">
                ...
                <p className="text-zinc-400">Board: <strong className="text-yellow-400">{selectedBoard}</strong></p>
                <p className="text-zinc-400">Subject: <strong className="text-emerald-400">{selectedSubject}</strong></p>
              </div>
